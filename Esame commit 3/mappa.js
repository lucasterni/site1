document.addEventListener('DOMContentLoaded', async () => {
  ...
  for (const [ticketKey, ticket] of tickets) {
    ...
    const coords = await resolveCoordinates(location, cache);
    if (!coords) continue;

    const key = `${coords[0].toFixed(3)}:${coords[1].toFixed(3)}`;
    const overlapIndex = usedSpots.get(key) || 0;
    usedSpots.set(key, overlapIndex + 1);
...
async function resolveCoordinates(location, cache) {
  ...
  try {
    const query = encodeURIComponent(location.replace('[unknown]', '').replace('(?)', '').trim());
    if (!query) {
